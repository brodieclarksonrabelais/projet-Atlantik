...
namespace projetAtlantik_Brodie
{
    public partial class FormAffichageTraversee : Form
    {
        ...
        private List<Traversees> GetLesTraverseesBateaux(int pNoLiaison, DateTime pDateTraversee)
        {
            MySqlConnection maCo;
            maCo = new MySqlConnection("server=localhost;user=root;database=atlantik2024;port=3306");
            string requete;
            ...
            try
            {
                ...
                requete = "Select notraversee, nom, dateheuredepart from traversee t inner join bateau b on (t.nobateau = b.nobateau) where noliaison = @noliaison and DATE(dateheuredepart) like DATE(@dateheuredepart);";
                maCde = new MySqlCommand(requete, maCo);
                ...
                while (jeuEnregistrements.Read())
                {
                    ...
                    Traversees trav = new Traversees(notraverse, dateheuredepart);
